...
export default class Todo extends Component {
    ...
    addTask() {
        ...
        if (!inputElem || !inputElem.value || this.state.tasks.indexOf(inputElem.value) !== -1) {
            return;
        }
        this.setState({
            ...this.state,
    ...
    removeTask(event: Event) {
        const taskToRemove = (event.target as HTMLElement).parentElement!.firstElementChild;
        this.setState({
            ...this.state,
            tasks: this.state.tasks.filter(task => task !== taskToRemove!.innerHTML),
        });
        this.elem.getElementsByTagName("input")[0].value = "";
    }

    events = {
        "click@button.add": this.addTask,
        "click@button.remove.all": this.removeTask,
    };
};